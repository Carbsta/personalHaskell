...
-- T-IF
chkCmd env (A.CmdIf {A.ciCondThens = ecs, A.ciMbElse = mc,
                     A.cmdSrcPos=sp}) = do
    es <- mapM (\e -> chkTpExp env e Boolean) (fst <$> ecs) -- env |- es : Boolean
    cs <- mapM (chkCmd env) (snd <$> ecs)                   -- env |- cs
    c3 <- mapM (chkCmd env) mc                              -- env |- c3
    return (CmdIf {ciCondThens = zip es cs, ciMbElse = c3, cmdSrcPos = sp})
-- T-WHILE
...
chkCmd env (A.CmdRepeat {A.crBody = c, A.crCond = e, A.cmdSrcPos = sp}) = do
    c' <- chkCmd env c                                  -- env |- c
    e' <- chkTpExp env e Boolean                        -- enc |- e : Boolean
...
-- T-LITCHR
infTpExp env e@(A.ExpLitChr {A.elcVal = h, A.expSrcPos = sp}) = do
    h' <- toMTChr h sp
    return (Character,                          -- env |- h : Character
            ExpLitChr {elcVal = h', expType = Character, expSrcPos = sp})
-- T-VAR
...
-- T-COND
infTpExp env (A.ExpCond {A.ecCond = e1, A.ecTrue = e2, A.ecFalse = e3,
                                                       A.expSrcPos = sp}) = do
    e1'       <- chkTpExp env e1 Boolean       -- env |- e1 : Boolean
    (t2, e2') <- infNonRefTpExp env e2         -- env |- e2 : t, not reftype(t)
    (t3, e3') <- infNonRefTpExp env e3         -- env |- e3 : t, not reftype(t)
    require (t2 == t3) sp ("True and False branches have different types: "
                           ++ show t2 ++ " and " ++ show t3 ++ " respectively.")
    case t2 == t3 of                           -- env(e2) == env(e3)
            True -> return (t2, ExpCond {ecCond = e1', ecTrue = e2',
                                         ecFalse = e3', expType = t2,
                                         expSrcPos = sp})
            False -> return (SomeType, ExpCond {ecCond = e1', ecTrue = e2',
                                                ecFalse = e3',
                                                expType = SomeType,
                                                expSrcPos = sp})
...
wellinit l (ExpCond {ecCond = e1,
                     ecTrue = e2,
                     ecFalse = e3}) = wellinit l e1 >>
                                      wellinit l e2 >>
                                      wellinit l e3
