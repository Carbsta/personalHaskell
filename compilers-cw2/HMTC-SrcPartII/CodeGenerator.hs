...
execute majl env n (CmdIf {ciCondThens = ts, ciMbElse = el}) = do
    lblOver <- newName
    mapM_ (\(e,c) -> do
                        lblNext <- newName
                        evaluate majl env e
                        emit (JUMPIFZ lblNext)
                        execute majl env n c
                        emit (JUMP lblOver)
                        emit (Label lblNext)) ts
    mapM_ (\c -> do {execute majl env n c}) el
    emit (Label lblOver)
...
evaluate majl env (ExpApp {eaFun = f, eaArgs = as}) = do
    ...
    evaluate majl env f
    emit CALLI
evaluate majl env (ExpCond {ecCond = b, ecTrue = e, ecFalse = f}) = do
    lblElse <- newName
    lblOver <- newName
    evaluate majl env b
