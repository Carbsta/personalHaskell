...
data Command
    ...
    | CmdIf {
          ...
          ciElsifs  :: [(Expression, Command)], -- ^ optional elsif branches
          ciElse    :: Maybe Command,           -- ^ Else-branch, if any
          cmdSrcPos :: SrcPos
      }
    -- | While-loop
    | CmdWhile {
...
data Expression
    ...
    | ExpCond {
          ecBool    :: Expression,      -- ^ Boolean expression
          ecTrue    :: Expression,      -- ^ If branch
          ecFalse   :: Expression,      -- ^ Else branch
          expSrcPos :: SrcPos
      }
