...
ppCommand n (CmdIf {ciCond = e, ciThen = c1, ciElsifs = els, ciElse = c2, cmdSrcPos = sp}) =
    indent n . showString "CmdIf" . spc . ppSrcPos sp . nl
    . ppExpression (n+1) e
    . ppCommand (n+1) c1
    . ppSeqPairs (n+1) (ppExpression, ppCommand) els
...
ppExpression n (ExpCond {ecBool = b, ecTrue = t, ecFalse = f, expSrcPos = sp}) =
